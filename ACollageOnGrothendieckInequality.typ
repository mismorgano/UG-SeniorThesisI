...
#theorem[
  ...
  $
  para todas las sucesiones de escalares ${s_i}$ y ${t_i}$ tsq $max abs(s_i) <=1$ y $max abs(t_i) <=1$. Entonces existe una constante universal $K_G$ tq
  $
...
#theorem[Desigualdad de Grothendieck][
  Existe una constante universal $K_G$ para la cual, dado cualquier Hs $H$, cualquier $n in NN$, y cualquier matriz escalar $(a_(i j))$ y cualesquiera vectores en $x_1, dots, x_n, y_1, dots, y_n in B_H$, tenemos que
  $
...
Una cuarta version (más general?) dada en @Garling_2007, 
...
$
donde $H$ es un Hs real o complejo. Al igual que antes $g(A)$ es simplemente es la norma del operador $T_A: #l^n_infinity (H) -> #l^m_1 (H)$, dado por $T_A (k) = (sum_(j=1)^n a_(i j) k_j)_(i=1)^m$,
para $k = (k_1, dots, k_n) in #l^n_infinity (H)$.
